...
#define TUBE_COLOR randColor2 
#define CAR_COLOR vec3(.4,.7,1.)
#define TURBINES_COLOR randColor
...
#define DETAIL .004
#define SPEED 8.
#define t (mod(time,500.)+10.)*SPEED
...
vec3 shade(in vec3 p, in vec3 dir, in vec3 n) {
	...
	// track lines
	if (pos.y<.5) col+=pow(max(0.,.2-abs(pos.x))/.2*abs(sin(pos.z*2.)),8.)*TUBE_COLOR*2.*(1.0+2.0*vuLow);
	pos.x=abs(pos.x);
	// fake AO for the tunnel's upper corners
	if(tunnel(pos.z)<0.)
		col*=1.-pow(max(0.5,1.-length(pos.xy+vec2(-FOLD*1.5,-.85))),5.)*max(0.,1.+pos.y);
	if (tubes(pos)<det) col=TUBE_COLOR*(1.0+2.0*vuLow); // hit tubes
	...
	vec3 tl=((tl1+tl2)*(.5+tubeinterval*.5))*TUBE_COLOR*(1.0+2.0*vuLow);//*(1.+tun*.5);
	...
	vec3 cl=TURBINES_COLOR*((diff+spec*.0)*cfalloff+cfalloff*.3)*(1.0+2.0*vuLow);
...
/*vec3 shadeplanet(vec3 pos, vec3 k) { 
	...
	// add some noise
	float noi= 0.0; //max(0.,texture2D(iChannel1,n.yz*.5).x-.6);
...

{
	...
	if (d<.5) { // hit surface
		...
		col+=tglow*TUBE_COLOR*pow(tubeinterval,1.5+(1.0+2.0*vuLow))*2.; // add tube glow
	...
	col+=TURBINES_COLOR*pow(abs(cglow),(1.0+2.0*vuLow))*.4;
	col+=TURBINES_COLOR*cglow*.15*(1.0+2.0*vuLow);
...
vec4 showtrack(vec2 p) {
	...
	float curs=pow(max(0.,1.-length(uv+vec2(0.,curpos))*2.),10.)*max(0.5,sin(time*10.))*2.;
...
void main(void)
{
	...
	vec2 mouse=vec2(0.0,0.0); //(iMouse.xy/resolution.xy-.5)*vec2(7.,1.5);
	vec3 iMouse=vec3(0.0);
	if (iMouse.z<1.) { // if no mouse, alternate rear and back cam
		mouse=vec2(sin(time)*.7,2.+sin(time*.2)*.22)
			*min(0.,sign(10.-mod(time,20.)));
	}
