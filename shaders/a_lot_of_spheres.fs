...
vec3 getSphereColor( vec2 grid ) {
	int gridIdx = int(mod(grid.x+grid.y*20.0,32));
	
	return (0.2+vuData[gridIdx]*3.0) * randColor; // (normalize( hash3( grid+vec2(43.12*grid.y,12.23*grid.x) ) ));
}
...
void main(void) {
	...
	// camera	
	vec3 ce = vec3( cos( 0.232*time) * 10., 6.+3.*cos(0.3*time), GRIDSIZE*(time/SPEED)+GRIDSIZE*timerKick );
	vec3 ro = ce;
