...
float map(in vec3 p)
{
	...
	for (float i = 0.0; i < 3.0; i++) {
		z.xz = rotate(z.xz, radians(i*7.0+(timerKick*20.0)));
		z.zy = rotate(z.yz, radians(i*7.0+(timerKick*20.0)));
...
vec3 hsv(in float h, in float s, in float v) {
	vec3 mVal = mix(vec3(1.0), clamp((abs(fract(h + vec3(3, 2, 1) / 3.0) * 6.0 - 3.0) - 1.0), 0.0 , 1.0), s) * v;
	return vec3((mVal.r+mVal.g+mVal.b)/3.0)*randColor;
}
...
vec3 intersect(in vec3 rayOrigin, in vec3 rayDir)
{
	...
	float d = 1.0;
	float iter = 0.0;
	float mind = 3.14159+sin(time*0.00000001)*0.000001;
