...
vec3 path(float ti) {
	vec3  p=vec3(sin(ti)+cos(ti*0.5),(1.-sin(ti*.5))*.5,ti*10.0)*.5;
	return p;
}
...
vec3 light(in vec3 p, in vec3 dir, in vec3 n, in float hid) {
	...
	if (hid>.5) {
		...
		vec3 c=planeVUMatrix(mod(p.zy+vec2(.4,0.2),vec2(1.))*2.0).xyz*2.;
		col+=c*abs(.01-mod(p.y-time*.1,.02))/.01*ao;
		col*=max(0.,1.-pow(length(p2.yz-vec2(.25,1.)),2.)*3.5)+c;
	} else{
		vec3 c=(planeVUMatrix(mod(p.zx*2.+vec2(0.5),vec2(1.))*2.0).xyz);
...

{
	...
	col+=((0.75)+vuLow*0.5)*lglow*exp(min(30.,totdist)*.02);
	return col; 
}
