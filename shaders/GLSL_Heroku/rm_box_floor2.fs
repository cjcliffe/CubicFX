...
void main(void){
  ...
  vec3 vrp=vec3(0,1,0); //Change camere view here
	float t = time*0.3125;
  vec3 prp=vec3(-sin(t)*16.0,4.0+sin(t)*1.0,cos(t)*4.0); //Change camera path position here
  ...
  if (f<maxd){
    ...
    fc = vec4((b*c+pow(b,16.0))*(0.85-f*.08),1.0);//simple phong LightPosition=CameraPosition
  }
  else fc=vec4(0,0,0,1); //background color
  fc.rb *= mod(gl_FragCoord.y, 2.0);
  gl_FragColor=fc; //background color
}