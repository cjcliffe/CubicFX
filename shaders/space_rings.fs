...
{
	float d=1024.0;
	float t = (5.0*timerKick+time)*object_speed_modifier+5.0;
...
vec3 object_material(vec3 p, vec3 d) //computes the material for the object
{
	...
	float post_light = pow(dot(r,normalize(-p))*.50+.5,40.0); //*post_light_occlusion;
		...
		float oa=.0;
		#endif
		float od = 1.0; //post_light_occlusion*.5+.5;
...
//	vec3 mouse = vec3(iMouse.xy/resolution.xy - 0.5,iMouse.z-.5);

	vec3 mouse = vec3(time/100.0,time/95.0,0.0)+timerKick/10.0;
	...
	float post_light_occlusion = occlusion(sp-md,md,4.0,1.0)/2.0;
	...
	color+=(0.5+vuLow)*light_color*(post_light+post_light_scifi);
