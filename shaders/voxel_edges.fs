...
float noise( in vec2 x )
{
	...
	return texture2D( tex0, (uv+118.4)/256.0, -100.0 ).x;
}
...
float mapTerrain( vec3 p )
{
	...
	float ft = fract( time );
	float it = floor( time );
	ft = smoothstep( 0.1, 1.0, ft );
	time = it + ft + timerKick; 
	float spe = 1.4;
...
//    vec2 mo = iMouse.xy / resolution.xy;
	...
	if( t>0.0 )
	{
        ...
        col = 2.0*texture2D( tex1,0.01*pos.xz ).zyx; 
        col += 0.8*vec3(0.1,0.3,0.4);
        col *= 0.5 + 0.5*texcube( tex2, 0.5*pos, nor ).x;
        col *= 1.0 - 0.75*(1.0-vvv)*www;
		
        // lighting
        float dif = clamp( dot( nor, lig ), 0.0, 1.0 ) * (vuLow+0.6);
