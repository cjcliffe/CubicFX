...
void main( void )
{
    ...
    float time = 5.0 + 0.4*time; // + 20.0*iMouse.x/resolution.x;

	/*
	freqs[0] = vuData[0]; //texture2D( iChannel0, vec2( 0.01, 0.25 ) ).x;
	freqs[1] = vuLow; // texture2D( iChannel0, vec2( 0.07, 0.25 ) ).x;
	freqs[2] = vuMid; // texture2D( iChannel0, vec2( 0.15, 0.25 ) ).x;
	freqs[3] = vuHigh; // texture2D( iChannel0, vec2( 0.30, 0.25 ) ).x;
*/
    // camera	
