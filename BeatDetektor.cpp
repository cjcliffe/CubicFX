...
void BeatDetektorVU::process(BeatDetektor *detector, float last_update, float current_bpm)
{
	float det_max = 0.0;
	...
	for (int i = 0; i < BD_DETECTION_RANGES; i++)
	{
		...
		if (det && det_val>1.0)
		{
			...
			if (det_val > 1.5) det_val = 1.5;
			
			if (det_val > vu_levels[i]) 
				vu_levels[i] = det_val;
			else if (current_bpm) vu_levels[i] -= 5.0*(vu_levels[i]*last_update);
		}
		else 
		{
			if (current_bpm) vu_levels[i] -= 7.0*(vu_levels[i]*last_update);
		}
